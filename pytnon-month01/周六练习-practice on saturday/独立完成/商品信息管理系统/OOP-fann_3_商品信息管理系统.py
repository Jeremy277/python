...
class ShoppingManagerController:
    ...
    def add_cart_info(self,cart_info):
        cart_info.id = self.__generate_cart_id()
        self.cart_list.append(cart_info)

    def __generate_cart_id(self):
        ShoppingManagerController.__card_id += 1
        return ShoppingManagerController.__card_id
    ...
    def settlement(self):
        ...
        for item in self.__cart_list:
            total_money += item.count * item.commodity.price
        return total_money
...
''')
    ...
    def __add_cart(self):
        while True:
            ...
            commodity = self.__manger.get_commodity(id)
            if commodity:
                count = int(input('请输入需要购买的数量：'))
                order = Cart_info_Model(commodity,count)
                self.__manger.add_cart_info(order)
                print('添加购物车成功')
                break
            else:
                print('请输入正确商品编号')

    def __input_cart_info(self):
        for item in self.__manger.cart_list:
            print(item.id,item.commodity.name,item.count,item.commodity.price)

    def __paying(self):
        total_money = self.__manger.settlement()
        print('请支付',total_money)
        money = int(input('请输入金额：'))
        if money >= total_money:
            print('支付成功找零',money-total_money)
